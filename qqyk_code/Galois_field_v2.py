...
伽罗华域运算
'''
primitive_polynomial_dict = {2:0b101,
                            3: 0b1011,
                            4: 0b10011,  # x**4  + x  + 1
...
class GF:
    ...
    def mul(self, a, b):##这个就是先看ab对应是2^x,2^y 所以a/b=2^(x-y),a*b=2^(x+y)
        a=self.check(a)
        b=self.check(b)
        return self.gfilog[(self.gflog[a] + self.gflog[b]) % (self.total-1)]
    ...
    def ex(self,a,x):
        a=self.check(a)
        return self.gfilog[(self.gflog[a]*x)%(self.total-1)]
    def check(self,a):
...
class GF_data:
    ...
    def __add__(self,b):
        return GF_data(self.gf.add(self,b),self.gf)
    def __eq__(self,b):
    ...
    def __mul__(self,b):
        return GF_data(self.gf.mul(self,b),self.gf)
    def __truediv__(self,b):
        return GF_data(self.gf.div(self,b),self.gf)
    def __str__(self):
